...
from src.minner.dl.abstract.callback import MLFlowTrackCallback
...
class MyTrainer(Trainer):
    ...
    def validation_loop(self, epoch):
        ...
        val_running_loss = 0
        val_writer = SummaryWriter('../storage/tb/val/test_mlflow/')
        ...
        print(f"Validation Epoch Loss :  { epoch_val_loss}")
...
def main():
    ...
    class Params:
        def __init__(self, batch_size, test_batch_size, epochs, init_lr, cuda):
    ...
    args = Params(32, 32, 3, 0.01, False)

    # TODO: Correct the UI dir path Problem
    # Need to be set first
    mlflow.set_tracking_uri("../storage/mlflow/")
    experiment = mlflow.set_experiment("Test1")
    # experiment = mlflow.get_experiment_by_name("Test1")
    with mlflow.start_run(experiment_id=experiment.experiment_id,
                          run_name="Hello mlflow") as run:
        ...
        trainer.start_to_train(train_data_loader=train_loader,
                            epochs=EPOCHS,
                            tb_path="../storage/tb/train/test_mlflow/",
                            earlystopping=WorseValLossEarlyStopping(3),
                            callback=MLFlowTrackCallback(),
